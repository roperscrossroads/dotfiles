...
get_fzf_output() {
  ...
  if command -v fd &>/dev/null; then
    # TODO: might want to make some modifications to this
  ...
  rg --line-number --no-heading --color=always --smart-case --glob '!**/.git/**' --glob '!node_modules/**' '' 2>/dev/null | \
    fzf --ansi --multi --delimiter : \
        --print-query \
        --preview '([[ -d {1} ]] && eza --color=always --long --all --header --icons --git {1} || bat --style=numbers --color=always --highlight-line {2} {1}) 2>/dev/null || echo "Preview not available"' \
